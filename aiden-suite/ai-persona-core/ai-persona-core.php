...
function aipc_render_settings_page() {
    ...
    <div class="wrap">
        ...
        <?php $active_tab = isset( $_GET['tab'] ) ? sanitize_key( $_GET['tab'] ) : 'dashboard'; ?>
        ...
        if ( $active_tab == 'dashboard' ) {
            ...
            echo '<div style="text-align: center;"><h2>' . (int)$num_agents . '</h2>' . esc_html__( 'Agents', 'ai-persona-core' ) . '</div>';
            echo '<div style="margin-left: 20px; padding-left: 20px; border-left: 1px solid #ddd; text-align: center;"><h2>' . (int)$num_personas . '</h2>' . esc_html__( 'Personas', 'ai-persona-core' ) . '</div>';
            echo '<div style="margin-left: 20px; padding-left: 20px; border-left: 1px solid #ddd; text-align: center;"><h2>' . (int)$num_pending . '</h2>' . esc_html__( 'Pending Reactions', 'ai-persona-core' ) . '</div>';
            echo '</div>';
            if ( ! get_transient( 'trm_demo_data_setup_complete' ) ) {
                echo '<div><h3>' . esc_html__( 'Get Started: Setup Demo Content', 'ai-persona-core' ) . '</h3><p>' . esc_html__( 'To see AiDen in action, you can create a rich set of sample users and personas. This will only run once.', 'ai-persona-core' ) . '</p>';
...
function aipc_render_scheduled_posting_field() {
    ...
    <label for="aiden_enable_scheduled_posting">
        <input type="checkbox" name="aiden_enable_scheduled_posting" id="aiden_enable_scheduled_posting" value="1" <?php checked( $enabled, 'on' ); ?> />
